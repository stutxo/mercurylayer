...
async fn bip448_sender_finishes_after_receiver_rotates_auth_key() -> Result<()> {
    ...
    let receiver = mercuryrustlib::sqlite_manager::get_wallet(&config.pool, &receiver.name).await?;
    let recipient_coin = receiver.coins.iter().find(|coin| coin.address == recipient)
        .context("recipient transfer coin is missing")?;
    let auth_pubkey = recipient_coin.auth_pubkey.clone();
    config.pool.close().await;

    assert_exit(
        &run_child(&sender_name, &statechain_id, &recipient, Some("transfer_msg_uploaded"))?,
        RESTART_EXIT,
        "uploaded transfer before local completion",
    )?;
    let interrupted = mercuryrustlib::client_config::load().await;
    assert!(mercuryrustlib::sqlite_manager::get_bip448_pending_transfer_signing(
        &interrupted.pool, &sender_name, &statechain_id,
    ).await?.is_some());
    let sender = mercuryrustlib::sqlite_manager::get_wallet(&interrupted.pool, &sender_name).await?;
    assert_eq!(sender.coins.iter().find(|coin| coin.statechain_id.as_deref() == Some(&statechain_id))
        .context("sender transfer coin is missing")?.status, CoinStatus::CONFIRMED);
    ...
    let accepted = mercuryrustlib::sqlite_manager::get_bip448_statechain(
        &interrupted.pool, &receiver_name, &statechain_id,
    ).await?;
    assert_eq!(accepted.latest_state_number, 2);
    let receiver = mercuryrustlib::sqlite_manager::get_wallet(&interrupted.pool, &receiver_name).await?;
    assert_eq!(receiver.coins.iter().find(|coin| {
        coin.statechain_id.as_deref() == Some(&statechain_id) && coin.status == CoinStatus::CONFIRMED
    }).context("receiver did not persist the accepted state-2 coin")?.status, CoinStatus::CONFIRMED);
    interrupted.pool.close().await;
    ...
    let recovered = mercuryrustlib::client_config::load().await;
    assert!(mercuryrustlib::sqlite_manager::get_bip448_pending_transfer_signing(
        &recovered.pool, &sender_name, &statechain_id,
    ).await?.is_none());
    let sender = mercuryrustlib::sqlite_manager::get_wallet(&recovered.pool, &sender_name).await?;
    assert_eq!(sender.coins.iter().find(|coin| coin.statechain_id.as_deref() == Some(&statechain_id))
        .context("sender transfer coin is missing after recovery")?.status, CoinStatus::IN_TRANSFER);
...
async fn get_encrypted_msg(client: &reqwest::Client, auth_pubkey: &str) -> Result<String> {
    get_encrypted_msgs(client, auth_pubkey).await?.into_iter().next()
...
async fn get_encrypted_msgs(client: &reqwest::Client, auth_pubkey: &str) -> Result<Vec<String>> {
    Ok(client.get(format!("{}/transfer/get_msg_addr/{auth_pubkey}", common::mercury::MERCURY_URL))
        .send().await?.json::<GetMsgAddrResponsePayload>().await?.list_enc_transfer_msg)
}
