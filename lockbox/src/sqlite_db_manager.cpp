...
CREATE TABLE lockbox_metadata (
    ...
    seed_verifier BLOB NOT NULL CHECK (
        ...
        void secure_database_file(const std::string& path, bool create) {
            ...
            try {
                ...
                if (!S_ISREG(status.st_mode) || status.st_nlink != 1) {
                    throw std::runtime_error(
                        "SQLite database path must be a regular file with one hard link."
                    );
                }
                if (status.st_uid != ::geteuid()) {
                    throw std::runtime_error(
                        "SQLite database file must be owned by the Lockbox user."
                    );
                }
        ...
        constexpr char kKeyupdateDomain[] = "BIP448/lockbox-keyupdate/v1\0";
