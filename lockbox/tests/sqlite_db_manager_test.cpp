...
std::array<unsigned char, Size> decode_lower_hex(const std::string& encoded) {
    require(encoded.size() == Size * 2, "Hex value has the wrong size.");
    std::array<unsigned char, Size> decoded{};
    const auto digit = [](char value) -> unsigned char {
        ...
        throw std::runtime_error("Hex value is not canonical lowercase.");
    };
    for (std::size_t index = 0; index < Size; ++index) {
        decoded[index] = static_cast<unsigned char>(
            (digit(encoded[index * 2]) << 4) | digit(encoded[index * 2 + 1])
        );
    }
    return decoded;
}

std::vector<unsigned char> canonical_blinded_session(unsigned char challenge_marker = 1) {
    std::vector<unsigned char> session(133, 0);
    session[0] = 0x9d;
    session[1] = 0xed;
    session[2] = 0xe9;
    session[3] = 0x17;
    session[68] = 1;
    session[100] = challenge_marker;
    session[132] = 1;
    return session;
}
...
void test_bip448_backend(const std::filesystem::path& directory) {
    ...
    db_manager::Bip448SignSecondRequest second;
    second.statechain_id = statechain_id;
    second.signing_id = first.signing_id;
    second.negate_seckey = 0;
    second.session = canonical_blinded_session();
    second.server_pubnonce =
        decode_lower_hex<db_manager::BIP448_PUBLIC_NONCE_SIZE>(first_result.value);
    second.expected_key_generation = 0;
    second.expected_server_pubkey = initial_key;
    const auto second_result = db_manager::execute_bip448_sign_second(
        second,
        const_cast<unsigned char*>(seed.data())
    );
    require(
        second_result.outcome == db_manager::Bip448Outcome::Applied &&
            second_result.value.size() == 64 &&
            second_result.actual_sig_count == 1,
        "Partial signature was not applied exactly once."
    );
    const auto second_replay = db_manager::execute_bip448_sign_second(
        second,
        const_cast<unsigned char*>(seed.data())
    );
    require(
        second_replay.outcome == db_manager::Bip448Outcome::ExactReplay &&
            second_replay.value == second_result.value,
        "Exact partial-signature retry did not replay."
    );
    auto conflicting_second = second;
    conflicting_second.session = canonical_blinded_session(2);
    require(
        db_manager::execute_bip448_sign_second(
            conflicting_second,
            const_cast<unsigned char*>(seed.data())
        ).outcome == db_manager::Bip448Outcome::OperationConflict,
        "Changed partial-signature challenge reused a nonce."
    );
    observed = db_manager::observe_bip448_state(statechain_id);
    require(
        observed.outcome == db_manager::Bip448Outcome::Applied &&
            observed.state.has_value() &&
            observed.state->sig_count == 1,
        "Partial-signature replay or conflict changed the signature count."
    );
    ...
    update.t2.back() = 2;
    update.x1.back() = 3;
    update.expected_sig_count = 1;
    update.expected_key_generation = 0;
    update.expected_server_pubkey = initial_key;
    auto wrong_count_update = update;
    wrong_count_update.operation_id = std::string(64, 'c');
    wrong_count_update.expected_sig_count = 0;
    require(
        db_manager::execute_bip448_keyupdate(
            wrong_count_update,
            const_cast<unsigned char*>(seed.data())
        ).outcome == db_manager::Bip448Outcome::SignatureCountMismatch,
        "Key update accepted the wrong signature count."
    );
    ...
    require(
        db_manager::delete_statechain(statechain_id).outcome ==
            db_manager::Bip448Outcome::Applied,
        "Exact statechain delete retry failed."
    );
    require(
